...
export async function readVideoJob(pool, key, sessionId, jobId, knownVersion = "") {
  ...
  const result = await pool.query(`
    SELECT auth.account,
      ...
      md5((selected.job - 'lockedAt')::text) AS version,
      CASE WHEN (
        auth.account->>'role' = 'admin'
        OR selected.job->>'createdBy' = auth.account->>'id'
      ) AND md5((selected.job - 'lockedAt')::text) IS DISTINCT FROM $4
...
export async function heartbeatVideoJob(pool, key, jobId, attempt) {
  const client = await pool.connect();
  try {
    await client.query("BEGIN");
    const result = await client.query(`
      SELECT (jobs.position - 1)::text AS position
      FROM app_state AS state,
        jsonb_array_elements(COALESCE(state.value->'videoAnalysisJobs', '[]'::jsonb)) WITH ORDINALITY AS jobs(job, position)
      WHERE state.key = $1 AND job->>'id' = $2 AND job->>'attempts' = $3
        AND job->>'status' IN ('running_player_detection', 'running_score_analysis')
      FOR UPDATE OF state
    `, [key, jobId, String(attempt)]);
    const position = result.rows[0]?.position;
    if (position != null) {
      await client.query(`UPDATE app_state
        SET value = jsonb_set(value, ARRAY['videoAnalysisJobs', $2, 'lockedAt'], to_jsonb($3::text)), updated_at = NOW()
        WHERE key = $1`, [key, position, new Date().toISOString()]);
    }
    await client.query("COMMIT");
