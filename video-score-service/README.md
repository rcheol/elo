...
GEMMA_PLAYER_FRAME_JPEG_QUALITY=10
...
- 대기 중인 worker의 조회는 DB에서 작업 존재 여부만 확인하며 전체 상태를 읽거나 저장하지 않습니다.
- 작업 배정은 기존 트랜잭션과 행 잠금으로 보호합니다. 상태 저장 시 변경된 최상위 항목만 전송합니다.
- 홈페이지는 보이는 탭에서 15초 간격으로 확인합니다. 이전 응답의 `version`을 보내면, 변경이 없을 때 이미지 대신 `{ "unchanged": true, "version": "..." }`만 받습니다.
- 이 최적화는 서버 배포와 홈페이지 새로고침으로 적용됩니다. 기존 worker와도 호환됩니다.
